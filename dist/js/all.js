...
angular.module("brawlApp", ['ui.router', 'ngAnimate']).config(['$stateProvider', '$urlRouterProvider', function ($stateProvider, $urlRouterProvider) {
  ...
  $stateProvider.state('home', {
    ...
    views: {
      'player-header': {
        // templateUrl: "/views/player-header/player-header.html",
        templateUrl: "/views/player-header.html" // controller: "rosterCtrl"
...
angular.module("brawlApp").controller("rosterCtrl", ['$scope', '$timeout', 'mainService', '$stateParams', function ($scope, $timeout, mainService, $stateParams) {
  ...
  mainService.getDbData(url).then(function (response) {
    ...
    $scope.currentTeamData = response; // return response;
  });
...
angular.module("brawlApp").controller("fightCtrl", ['$scope', 'mainService', '$stateParams', function ($scope, mainService, $stateParams) {
  ...
  generateGameStats($scope.rightPlayerGameStats); //   function forceDescriptives(list, mean, sd) {
      ...
      return function (p, J, u) {
        ...
        function O(a, b) {
          ...
          for (var c = [], d = 0; d < a.length; d++) {
            var f = a[d];
            if ("ng-" !== f.substring(0, 3)) for (var e = 0; e < b.length; e++) {
              if (f === b[e]) {
                c.push(f);
                break;
              }
